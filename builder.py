            ...
            with gr.Tabs():
                ...
                for group, desc, variables in variable_groups:
                    with gr.TabItem(group):
                        gr.Markdown(
                            desc
                            + "\nYou can set these to one of the dropdown values, or"
                            " clear the dropdown to set a custom value."
                        )
        ...
        with gr.Column(scale=6, elem_id="app"):
            with gr.Column(variant="panel"):
                ...
                with gr.Accordion("Upload to Hub", open=False):
                    gr.Markdown(
                        "You can save your theme on the Hugging Face Hub. HF API write"
                        " token can be found"
        ...
        def upload_to_hub(data):
            try:
                ...
                return (
                    gr.Markdown(
                        value=(
                            f"Theme uploaded [here!]({theme_url})! Load it as"
                            f" `gr.Blocks(theme='{space_name}')`"
                        ),
                        visible=True,
                    ),
                    "Upload to Hub",
